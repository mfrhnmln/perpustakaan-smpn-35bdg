const AuthorFilter = ({
  searchQuery, // Nilai input pencarian (controlled component)
  setSearchQuery, // Setter untuk mengubah nilai pencarian (biasanya dari parent state)
  totalAuthors, // Total penulis (bisa semua atau hasil filter)
  ...
  return (
    ...
    <div className="flex flex-col lg:flex-row lg:items-center gap-4 py-5">
      {/* Input pencarian berdasarkan nama penulis */}
      <div className="text-paragraph relative w-full lg:max-w-md">
        <input
          name="search"
          type="text"
          placeholder="Cari nama penulis..." // Placeholder saat input kosong
      ...
      {/* Informasi total data Penulis */}
      <p className="text-paragraph w-full ml-auto text-right lg:max-w-40">
        Total: <span className="text-subtitle font-bold">{totalAuthors}</span>{" "}
        data
      </p>
    </div>
  );
